...
public class CoordManager : MonoBehaviour
{
    ...
    void Update()
    {
        if (!_coordFlag)
        {
            if (_anchorInstances.Count == 4)
            {
                SetWorldToAchorCoord();
                _coordFlag = true;
            }
        }
    }

    private void SetWorldToAchorCoord()
    {
        Vector3 vec1 = _anchorInstances[0].transform.position;
        Vector3 vec2 = _anchorInstances[1].transform.position;
        Vector3 vec3 = _anchorInstances[2].transform.position;
        Vector3 vec4 = _anchorInstances[3].transform.position;
        vec1.y = 0f;
        vec2.y = 0f;
        vec3.y = 0f;
        vec4.y = 0f;

        Vector3 center = (vec1 + vec2 + vec3 + vec4) / 4f;
        Vector3 right = ((vec1 - vec2) + (vec4 - vec3)) / 2f;
        Vector3 forward = ((vec1 - vec4) + (vec2 - vec3)) / 2f;
        Vector3 up = Vector3.up;

        _world.transform.position = center;
        _world.transform.rotation = Quaternion.LookRotation(forward, up);
    }
